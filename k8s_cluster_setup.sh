...
install_vllm() {
  ...
  # Created unconditionally so the Helm values can always reference it.
  # If the token is empty the secret holds an empty string — harmless for
  # public models, required for gated ones (Llama, Gemma, Mistral, etc.).
  if [[ -n "${VLLM_HF_TOKEN:-}" ]]; then
    info "Creating HuggingFace token Secret in namespace ${VLLM_NAMESPACE}..."
    kubectl create secret generic vllm-hf-token \
      --namespace "$VLLM_NAMESPACE" \
      --from-literal=token="${VLLM_HF_TOKEN}" \
      --dry-run=client -o yaml | kubectl apply -f -
    log "HuggingFace token Secret created/updated."
  else
    warn "No HuggingFace token configured — gated models (Llama, Gemma, Mistral) will fail."
    warn "To add a token later:  kubectl create secret generic vllm-hf-token \\"
    warn "  -n ${VLLM_NAMESPACE} --from-literal=token=<your-hf-token>"
  fi

  # ── Model cache PVC ────────────────────────────────────────────────────────
  # Resolve the storage class: prefer NFS if installed, else cluster default.
  local storage_class=""
  if [[ "${INSTALL_NFS:-false}" == "true" && -n "${NFS_STORAGE_CLASS:-}" ]]; then
    if kubectl get storageclass "${NFS_STORAGE_CLASS}" &>/dev/null; then
      storage_class="$NFS_STORAGE_CLASS"
      info "Using NFS StorageClass '${storage_class}' for model cache PVC."
    fi
  fi
  if [[ -z "$storage_class" ]]; then
    storage_class=$(kubectl get storageclass \
      ...
      2>/dev/null | awk '$2=="true"{print $1; exit}')
    [[ -n "$storage_class" ]] && \
      info "Using cluster default StorageClass '${storage_class}' for model cache PVC."
  fi
  ...
  if [[ "${VLLM_REUSE_PVC:-false}" == "true" ]]; then
    if $pvc_exists; then
      ...
      warn "VLLM_REUSE_PVC=true but PVC '${VLLM_PVC_NAME}' does not exist — creating it."
  ...
  if [[ "${VLLM_REUSE_PVC:-false}" != "true" ]]; then
    ...
    [[ -n "$storage_class" ]] && storage_class_line="  storageClassName: ${storage_class}"

    info "Creating model cache PVC '${VLLM_PVC_NAME}' (${VLLM_STORAGE_SIZE})..."
    kubectl apply -f - <<VLLMPVC
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: ${VLLM_PVC_NAME}
...
${storage_class_line}
VLLMPVC
    log "Model cache PVC '${VLLM_PVC_NAME}' created."
  fi
  ...
  # Build extra args list — split on whitespace into a YAML sequence
  local extra_args_yaml=""
  if [[ -n "${VLLM_EXTRA_ARGS:-}" ]]; then
    extra_args_yaml="  extraArgs:"$'\n'
    for arg in ${VLLM_EXTRA_ARGS}; do
      extra_args_yaml+="    - \"${arg}\""$'\n'
    done
  fi

  # HF token env injection — only if secret exists
  local hf_env_yaml=""
  if [[ -n "${VLLM_HF_TOKEN:-}" ]]; then
    hf_env_yaml='  env:
  ...
  # ── Estimate probe timing from model size ─────────────────────────────────
  # Large models (70B+) can take 20+ minutes to load from disk on first start.
  # We scale the startup probe budget by GPU count as a rough proxy for model
  # size: more GPUs → larger model → more loading time needed.
  # Formula:
  #   initialDelaySeconds : let the engine start the HTTP server first
  #   failureThreshold    : how many consecutive failures are tolerated
  #   periodSeconds       : how often to probe
  #   Budget = initialDelaySeconds + (failureThreshold × periodSeconds)
  #
  # Router startup probe: the router's /health endpoint tries to reach the
  # serving engine backends. It must not fail until the engine is truly up.
  # We use the same budget as the engine but add an extra initial delay so the
  # router doesn't start probing until the engine has had time to boot.
  local gpu_count="${VLLM_GPU_COUNT:-1}"
  local engine_init_delay=$(( 60 + gpu_count * 30 ))   # 90s for 1 GPU, 120s for 2, …
  local engine_failure_thresh=60                        # 60 × 10s = 600s window
  local engine_period=10

  # Router: starts probing its own HTTP port first, which comes up quickly.
  # The router's liveness/readiness checks its ability to reach backends only
  # on the /health path — give it the same generous startup window as the engine
  # so that a slow-loading model doesn't restart the router pod.
  local router_init_delay=$(( engine_init_delay + 30 )) # always after the engine
  local router_failure_thresh=60
  local router_period=10

  local values_file="/tmp/vllm-values-$$.yaml"
  cat > "$values_file" <<VLLMVALS
# vLLM production stack values — generated by k8s_cluster_setup.sh
# Model: ${VLLM_MODEL}
#
# Probe timings are sized to accommodate model download and load time.
# Engine startup budget : ${engine_init_delay}s + (${engine_failure_thresh} × ${engine_period}s) = $(( engine_init_delay + engine_failure_thresh * engine_period ))s
# Router startup budget : ${router_init_delay}s + (${router_failure_thresh} × ${router_period}s) = $(( router_init_delay + router_failure_thresh * router_period ))s
...
servingEngineSpec:
  ...
  # ── Startup probe: long window covers model download + load time ──────────
  # The vLLM HTTP server only starts listening on :8000 AFTER the model is
  # fully loaded. Without a generous startup probe the pod is killed and
  # restarted in a loop ("Startup probe failed: connection refused").
  startupProbe:
    httpGet:
      path: /health
      port: 8000
  ...
  # ── Liveness / readiness: only active after startup probe succeeds ────────
  livenessProbe:
    httpGet:
      path: /health
      port: 8000
  ...
  # Mount model cache PVC at the HuggingFace hub cache path
  volumes:
    - name: model-cache
...
router:
  # ── Router startup probe: must not fire until backends are ready ──────────
  # The router's /health endpoint returns 503 when no serving engine backends
  # have registered yet. Give it the same generous window as the engine so the
  # router pod is never restarted just because the model is still loading.
  startupProbe:
    httpGet:
      path: /health
      port: 8080
  ...
  service:
    type: NodePort
    nodePort: ${VLLM_NODEPORT}
VLLMVALS

  # ── Deploy via Helm ────────────────────────────────────────────────────────
  # --wait is intentionally omitted: it enforces the readiness probe passing
  # within --timeout, which is impossible during a first-time model download.
  # We instead run our own polling loop below which gives human-readable
  # progress and distinguishes "still loading" from "actually broken".
  info "Deploying vLLM production stack (model: ${VLLM_MODEL})..."
  helm upgrade --install vllm-stack vllm/vllm-stack \
    --namespace "$VLLM_NAMESPACE" \
    --values "$values_file" \
    --timeout 5m || {
      error "Helm install failed (chart error, not a probe timeout)."
      rm -f "$values_file"
      exit 1
    }
  ...
  # Poll until both the router and at least one engine pod are Running, or
  # until we detect a hard failure (CrashLoopBackOff / ImagePullBackOff / Error).
  ...
  info "Waiting for vLLM pods to become ready (up to ${poll_max}s)..."
  info "Model download and load time depends on model size and network speed."

  while (( elapsed < poll_max )); do
    local router_status engine_status
    router_status=$(kubectl get pods -n "$VLLM_NAMESPACE" \
      --no-headers 2>/dev/null \
      | awk '/router/ {print $3}' | head -1)
    engine_status=$(kubectl get pods -n "$VLLM_NAMESPACE" \
      --no-headers 2>/dev/null \
      | awk '!/router/ && NF>0 {print $3}' | head -1)

    # Detect hard failures immediately — no point waiting further
    for status in "$router_status" "$engine_status"; do
      case "$status" in
        CrashLoopBackOff|ImagePullBackOff|ErrImagePull|Error|OOMKilled)
          error "vLLM pod entered failed state: ${status}"
          error "Router pod:  $(kubectl get pods -n "$VLLM_NAMESPACE" --no-headers 2>/dev/null | awk '/router/')"
          error "Engine pods: $(kubectl get pods -n "$VLLM_NAMESPACE" --no-headers 2>/dev/null | awk '!/router/ && NF>0')"
          error "Recent events:"
          kubectl get events -n "$VLLM_NAMESPACE" \
            --sort-by='.lastTimestamp' 2>/dev/null | tail -10 | tee -a "$LOG_FILE" || true
          error "Deployment left in place for diagnosis."
          error "Logs: kubectl logs -n ${VLLM_NAMESPACE} -l app=vllm-stack --previous"
          return 1
          ;;
      esac
    done
    ...
    if $router_ready && $engine_ready; then
      log "All vLLM pods are Running after ${elapsed}s."
      break
    fi

    info "  [${elapsed}s] router=${router_status:-Pending}  engine=${engine_status:-Pending} — waiting..."
    sleep $poll_interval
    elapsed=$(( elapsed + poll_interval ))
  done

  if ! $router_ready || ! $engine_ready; then
    warn "vLLM pods did not reach Running within ${poll_max}s."
    warn "This is normal for very large models — they may still be downloading."
    warn "Monitor with: kubectl get pods -n ${VLLM_NAMESPACE} -w"
    warn "Engine logs:  kubectl logs -n ${VLLM_NAMESPACE} -l app=vllm-stack -c vllm -f"
  ...
  info "OpenAI-compat API:  http://${CONTROL_PLANE_IP}:${VLLM_NODEPORT}/v1"
  info "Model cache PVC:    ${VLLM_PVC_NAME} (${VLLM_STORAGE_SIZE:-reused})"
  info ""
  info "Test with:"
  info "  curl http://${CONTROL_PLANE_IP}:${VLLM_NODEPORT}/v1/models"
  info ""
  info "To swap model after deployment:"
  info "  helm upgrade vllm-stack vllm/vllm-stack -n ${VLLM_NAMESPACE} \\"
  info "    --reuse-values --set servingEngineSpec.model=<huggingface-model-id>"
}
...
if [[ "${1:-}" == "--uninstall" ]]; then
  ...
  case "$2" in
    ssh)        setup_ssh_keys ;;
    prep)       prepare_all_nodes ;;
    nvidia)     install_nvidia_drivers ;;
    k8s-bins)   install_k8s_binaries ;;
    init)       init_control_plane ;;
    cni)        install_cni ;;
    workers)    join_workers ;;
    helm)       install_helm ;;
    monitoring) install_monitoring ;;
    gpu-op)     install_gpu_operator ;;
    nfs)        install_nfs_provisioner ;;
    dashboard)  install_dashboard ;;
    vllm)       install_vllm ;;
    verify)     verify_cluster ;;
    uninstall)  uninstall_cluster ;;
    *)          error "Unknown step: $2"; exit 1 ;;
  esac
else
  main "$@"
fi
